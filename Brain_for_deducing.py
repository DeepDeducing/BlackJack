...
class Brain(object):
    ...
    def generate_values_for_each_layer(self, input):
        ...
        binomial                  = np.atleast_2d(np.random.binomial(1, 1 - 0.2, size=(100, 20)))                                      # <<<<<<<<<<<<
        ...
        binomial                  = np.atleast_2d(np.random.binomial(1, 1 - self.drop_rate, size=(100,self.network_size[1])))         # <<<<<<<<<<<<
        ...
        for i in range(self.number_of_layers - 3):
            ...
            binomial              = np.atleast_2d(np.random.binomial(1, 1 - self.drop_rate, size=(100,self.network_size[i + 2])))     # <<<<<<<<<<<<
